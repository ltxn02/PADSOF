...
public class Catalog {
	...
	public Catalog(ArrayList<Category> categories, ArrayList<AgeRange> ageRanges, ArrayList<NewProduct> products) {
		...
		this.productsOnSale = products != null ? products : new ArrayList<>();
		this.gamesByAge = new HashMap<>();

		// Sincronizar el mapa gamesByAge al cargar datos existentes
		this.organiseGamesByAgeRange();
	}
	...
	public void addAgeRange(int min, int max) {
		...
		this.gamesByAge.putIfAbsent(a, new ArrayList<>());
	}
	...
	public void organiseGamesByAgeRange() {
		...
		this.gamesByAge.clear();
		for(AgeRange age : ageRanges) {
			this.gamesByAge.put(age, new ArrayList<>());
		}
	...
	public void addProductOnSale(ItemType itemType, Map<String, Object> data) {
		try {
			...
			if(p != null) {
				...
				// CORRECCIÓN: Cast correcto a ArrayList<String>
				@SuppressWarnings("unchecked")
				ArrayList<String> picturePath = (ArrayList<String>) data.get("picturePath");
				...
				switch(itemType) {
					case COMIC: {
						...
						this.productsOnSale.add(new Comic(
								...
								(Integer) data.get("nPages"), (String) data.get("publisher"),
								(Integer) data.get("publicationYear"), authors));
						break;
					}
					case GAME: {
						@SuppressWarnings("unchecked")
						...
						Game newGame = new Game(
								name, description, price, picturePath, stock, categories, reviews, discount,
								(Integer) data.get("nPlayers"), mechanics, (AgeRange) data.get("ageRange"));
						this.productsOnSale.add(newGame);
						this.organiseGame(newGame); // Organizar al añadir
						break;
					}
					case FIGURINE: {
						this.productsOnSale.add(new Figurine(
								name, description, price, picturePath, stock, categories, reviews, discount,
								(Double) data.get("height"), (Double) data.get("width"),
								(Double) data.get("depth"), (String) data.get("material"),
								(String) data.get("franchise")));
						break;
					}
					case PACK: {
						@SuppressWarnings("unchecked")
						ArrayList<NewProduct> productsInPack = (ArrayList<NewProduct>) data.get("products");
						this.productsOnSale.add(new Pack(
								name, description, price, picturePath, stock, categories, reviews, productsInPack));
						break;
					}
				}
			}
	...
	private void addAgeRange(AgeRange ageRange) {
		for(AgeRange a: this.ageRanges) {
			if(a.equalTo(ageRange)) return;
		}
		this.ageRanges.add(ageRange);
		this.gamesByAge.putIfAbsent(ageRange, new ArrayList<>());
	}

	private void organiseGame(Game p) {
		AgeRange gameAge = p.getAgeRange();
		this.addAgeRange(gameAge); // Asegura que el rango existe

		for(AgeRange age : this.ageRanges) {
			// Un juego de "10 años" debe aparecer si filtras por "8 años" (si el rango lo permite)
			if(age.containedIn(gameAge) || age.equalTo(gameAge)) {
				List<Game> gamesList = this.gamesByAge.get(age);
				if(gamesList != null && !gamesList.contains(p)) {
					gamesList.add(p);
				}
			}
		}
	...
	private void validateData(ItemType itemType, Map<String, Object> data) {
		...
		this.checkField(data, "picturePath", ArrayList.class); // CORRECCIÓN: Es ArrayList
		this.checkField(data, "stock", Integer.class);

		if(data.get("discount") != null) {
			// Usamos IDiscount para que acepte cualquier implementación
			if(!(data.get("discount") instanceof IDiscount)) {
				throw new IllegalArgumentException("discount must implement IDiscount");
			}
		}
	...
	private void checkField(Map<String, Object> data, String key, Class<?> expectedType) {
		...
		if(!expectedType.isInstance(value)) {
			throw new IllegalArgumentException("Invalid class: " + key + " must be of " + expectedType.getSimpleName());
		}
	}
}